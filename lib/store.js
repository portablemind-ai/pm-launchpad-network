...
//   founderTeams   the team(s) each founder is on, per workspace:party — remembered so the lookup is
//                  one roster read; membership is still verified against the platform
//   founderAgents  the agents each accelerator's admin made available to founders
//   founderForms   the custom-object forms each accelerator's founders fill in (and one/many)
//   assistantChats each founder's private assistant conversation per agent, with a digest of the
//                  AI context last written to it (so it is rewritten only when it changes)
