...
function createFounderWorkLanes({ sessions, store, config, founders, forms }) {
  ...
  // Conversation names are unique per type within a workspace and must match [A-Za-z0-9_-]{1,128}
  // (the platform slugs anything else). So the name is built valid and deterministic from the
  // agent and the founder's party id: founders who share a name never collide, a renamed founder
  // keeps their chat, and the chat can always be found again — the platform stays the source of
  // truth, this app's saved id is only a shortcut.
  const chatName = (s, agent) => `${agent.name}_founder-${s.user.party_id}`.replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 128);
  ...
  // Returns { id, systemPrompt } — the AI context the platform actually holds for it.
  async function ensureConversation(s, agent, context) {
    ...
    if (id) {
      ...
      if (seen.ok) systemPrompt = (seen.body.llm_conversation || {}).system_prompt || null;
      else id = null; // deleted, or no longer ours
    }
    if (!id) {
      ...
      if (existing) {
        id = existing.id;
        systemPrompt = existing.system_prompt || null;
        rememberChat(key, { id, context: null });
      } else {
        ...
        systemPrompt = made.body.llm_conversation.system_prompt || null;
        rememberChat(key, { id, context: systemPrompt === context ? digestOf(context) : null });
      }
    }
  ...
  // Keep the conversation's AI context current. `actual` is what the platform holds (known when
  // the assistant is opened); otherwise the digest of what this server last confirmed writing is
  // compared — no read per message. The founder owns the conversation, so their session may update it.
  async function refreshContext(s, agent, id, context, actual) {
    const key = chatKey(s, agent);
    const digest = digestOf(context);
    const known = chatFor(key);
    const current = actual !== undefined ? actual === context : (known && known.id === id && known.context === digest);
    if (current) {
      if (!known || known.context !== digest) rememberChat(key, { id, context: digest });
      return;
    }
    const r = await call(s, `/llm_conversations/${id}`, { method: 'PUT', body: { llm_conversation: { system_prompt: context } } });
    // Remember it only once the platform confirms it stored it (an older platform accepts the
    // request but ignores the field — remembering that would stop us writing it once supported).
    const stored = r.ok && (r.body.llm_conversation || {}).system_prompt === context;
    if (stored) rememberChat(key, { id, context: digest });
  }
  ...
  async function openAssistant(req, res, url, agentId) {
    ...
    const { id, systemPrompt } = await ensureConversation(s, agent, context);
    await refreshContext(s, agent, id, context, systemPrompt); // verified against the platform
  ...
  async function sendToAssistant(req, res, url, agentId) {
    ...
    if (known) {
      ...
      const ensured = await ensureConversation(s, agent, context);
      id = ensured.id;
      await refreshContext(s, agent, id, context, ensured.systemPrompt);
    }
