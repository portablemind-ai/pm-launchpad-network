...
function createFounderLanes({ pm, sessions, store, secrets, config, auth }) {
  ...
  async function me(req, res) {
    ...
    sendJson(res, 200, {
      ...
      app_url: config.pmAppUrl || '',
      sso_available: !!(config.pmAppUrl && secrets.get(s.workspace, 'sso_key')),
    });
  }
  ...
  return { signup, me, channel, joinLink, joinInfo, ssoGo };
}
