...
function createFounderLanes({ pm, sessions, store, secrets, config, auth }) {
  ...
  async function signup(req, res) {
    ...
    if (created.party_id && r.body.team_id) rememberTeams(workspace, created.party_id, [{ id: Number(r.body.team_id), name: join ? join.n : startup }]);
  ...
  // The team(s) a founder belongs to, remembered when this app puts them there (sign-up, join
  // link) or first finds them — so the usual lookup is ONE roster read of their own team, not a
  // scan of every team in the accelerator. Membership is still re-verified against the platform.
  const teamsKey = (workspace, partyId) => `${workspace}:${partyId}`;
  function rememberTeams(workspace, partyId, teams) {
    store.update((st) => { st.founderTeams ||= {}; st.founderTeams[teamsKey(workspace, partyId)] = teams.map((t) => ({ id: t.id, name: t.name })); });
  }
  function forgetTeams(workspace, partyId) {
    store.update((st) => { if (st.founderTeams) delete st.founderTeams[teamsKey(workspace, partyId)]; });
  }

  // Which teams is the VERIFIED caller on? A founder cannot read team membership themselves, so
  // rosters are read with the provisioning key and matched on the caller's own party id — taken
  // from the server-side session, never from the browser. Any fault aborts (fail closed).
  // Cached per person for a few minutes: the key is metered on the accelerator's plan (guide §6.4).
  async function callerTeams(s) {
    ...
    const isMember = (members) => members.some((m) => String(m.party_id) === String(s.user.party_id));

    let mine = [];
    const known = (store.state.founderTeams || {})[teamsKey(s.workspace, s.user.party_id)] || [];
    for (const t of known) {
      const members = await roster(s.workspace, key, t.id);
      if (isMember(members)) mine.push({ id: t.id, name: t.name, members });
    }
    if (!mine.length) {
      // Nothing remembered (a founder from before this app recorded teams), or no longer on the
      // remembered team: find them the slow way once, and remember the answer.
      if (known.length) forgetTeams(s.workspace, s.user.party_id);
      ...
      if (mine.length) rememberTeams(s.workspace, s.user.party_id, mine);
    }
